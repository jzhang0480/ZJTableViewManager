...
class CameraMan {
    ...
    func setupDevices() {
        ...
        AVCaptureDevice
            .devices()
            .filter {
                $0.hasMediaType(AVMediaType.video)
            }.forEach {
                switch $0.position {
                case .front:
    ...
    private func start() {
        ...
        guard let input = (startOnFrontCamera) ? frontCamera ?? backCamera : backCamera, let output = stillImageOutput else { return }
    ...
    func configurePreset(_ input: AVCaptureDeviceInput) {
        for asset in preferredPresets() {
            if input.device.supportsSessionPreset(AVCaptureSession.Preset(rawValue: asset)) && session.canSetSessionPreset(AVCaptureSession.Preset(rawValue: asset)) {
                session.sessionPreset = AVCaptureSession.Preset(rawValue: asset)
                return
            }
        }
    }
