...
open class SKPhotoBrowser: UIViewController {
    ...
    internal lazy var pagingScrollView: SKPagingScrollView = .init(frame: self.view.frame, browser: self)
    ...
    @objc func panGestureRecognized(_ sender: UIPanGestureRecognizer) {
        ...
        if sender.state == .ended {
                ...
                || zoomingScrollView.center.y < viewHalfHeight - minOffset
            {
                ...
                let velocityY: CGFloat = .init(0.35) * sender.velocity(in: view).y
                let finalX: CGFloat = firstX
                let finalY: CGFloat = viewHalfHeight

                let animationDuration: Double = .init(abs(velocityY) * 0.0002 + 0.2)
